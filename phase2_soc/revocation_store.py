...
def revoke_token(jti: str) -> None:
    """
    Mark a token's unique ID (JTI) as revoked in Redis.
...
def is_revoked(jti: str) -> bool:
    ...
    Returns:
        True if revoked or if Redis is unreachable (fail-closed).
    """
    if _r is None:
        return True
    ...
    try:
        ...
        print(f"[ERROR] Redis sismember failed: {exc}")
        return True
...
def quarantine_agent(agent_id: str) -> None:
    ...
    Args:
        agent_id: The ID of the agent to quarantine.
    """
    if _r is None:
...
def is_quarantined(agent_id: str) -> bool:
    ...
    Returns:
        True if quarantined or if Redis is unreachable (fail-closed).
    """
    if _r is None:
        return True
